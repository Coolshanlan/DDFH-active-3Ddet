...
class Detector3DTemplate(nn.Module):
    ...
    def post_processing(self, batch_dict):
        ...
        if self.model_cfg['ROI_HEAD'].get('SAMPLING_ROUND',None):
            rcnn_cls_std = torch.mean(torch.var(torch.sigmoid(batch_dict['rcnn_cls']), 0).view(batch_size, -1, 1),-1) if 'rcnn_cls' in batch_dict.keys() and len(batch_dict['rcnn_cls'].shape) > 2 else None
            rcnn_reg_std = torch.mean(torch.var(batch_dict['rcnn_reg'], 0).view(batch_size, -1, 7)[...,-4:-1],-1) if 'rcnn_reg' in batch_dict.keys() and len(batch_dict['rcnn_cls'].shape) > 2 else None
        else: 
            rcnn_cls_std, rcnn_reg_std = None, None
        ...
        for index in range(batch_size):
            ...
            batch_rcnn_cls_std = rcnn_cls_std[batch_mask, :] if rcnn_cls_std is not None else None
            batch_rcnn_reg_std = rcnn_reg_std[batch_mask, :] if rcnn_reg_std is not None else None
            ...
            if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS:
                ...
                if 'shared_features' in batch_dict.keys():    
                    final_embedding = shared_features.view(batch_size,-1,shared_features.shape[-2],shared_features.shape[-1])
                    final_embedding = final_embedding[batch_mask][selected]
                
                final_rcnn_reg_std=None
                final_rcnn_cls_std=None
                if batch_rcnn_cls_std != None:
                    final_rcnn_cls_std = batch_rcnn_cls_std[selected]
                    final_rcnn_reg_std = batch_rcnn_reg_std[selected]
            ...
            record_dict = {
                ...
                'rcnn_cls':rcnn_cls,
                'rcnn_reg':rcnn_reg,
                'spatial_features_2d':batch_dict['spatial_features_2d'][batch_mask],
                ...
                'pred_rcnn_cls_std':final_rcnn_cls_std,
                'pred_rcnn_reg_std':final_rcnn_reg_std,
                'pred_embeddings':final_embedding,
